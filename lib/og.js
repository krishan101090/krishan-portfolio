export function renderOgImage(profile) {
  ...
  const jobTitle = profile?.person?.currentRole || profile?.person?.jobTitle || 'Software Engineer'
  const company = profile?.person?.currentCompany || 'McKinsey & Company'
  const tagline =
    profile?.person?.tagline ||
    'Professional portfolio — frontend architecture, React, Next.js, and AI-powered product engineering.'
  const availability =
