...
function experience(profile) {
  const lines = (profile.timeline || []).slice(-6).map((t) => {
...
function skills(profile) {
  ...
  return `Krishan's core stack includes ${list}. He specializes in React, Next.js, enterprise frontend architecture, performance, accessibility, and AI-assisted delivery.`
}

function achievements(profile) {
  const items = (profile.achievements?.items || []).slice(0, 4)
